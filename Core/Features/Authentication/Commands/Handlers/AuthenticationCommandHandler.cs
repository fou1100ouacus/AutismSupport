...
            var signInResult = await _signInManager.CheckPasswordSignInAsync(user, request.Password, false);
...

            {
                ...
                if (signInResult.IsLockedOut) return BadRequest<JwtAuthResult>("User is locked out");

                if (signInResult.IsNotAllowed) return BadRequest<JwtAuthResult>("User is not allowed to sign in");

                if (signInResult.RequiresTwoFactor) return BadRequest<JwtAuthResult>("Two-factor authentication required");
